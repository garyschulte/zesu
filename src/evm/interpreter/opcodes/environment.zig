...
/// Environment opcodes (block/tx info, calldata, code access), comptime-generic
/// over DB — see host.zig's Host(DB) doc comment. Most of these never touch
/// Host at all (they read interpreter.input only) but still live in this
/// namespace since they're entries in the same DB-bound dispatch table.
pub fn Ops(comptime DB: type) type {
    return struct {
        ...
        pub fn opAddress(ctx: *InstructionContext(DB)) void {
        ...
        pub fn opCaller(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasSpace(1)) {
                ctx.interpreter.halt(.stack_overflow);
                return;
        ...
        pub fn opCallvalue(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasSpace(1)) {
                ctx.interpreter.halt(.stack_overflow);
                return;
        ...
        pub fn opCalldatasize(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasSpace(1)) {
                ctx.interpreter.halt(.stack_overflow);
                return;
        ...
        pub fn opCalldataload(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(1)) {
        ...
        pub fn opCalldatacopy(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(3)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// Stack: [] -> [size]   Gas: 2 (G_BASE, dispatch)
        pub fn opCodesize(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasSpace(1)) {
                ctx.interpreter.halt(.stack_overflow);
                return;
        ...
        pub fn opCodecopy(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(3)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// Stack: [] -> [size]   Gas: 2 (G_BASE, dispatch)
        pub fn opReturndatasize(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasSpace(1)) {
                ctx.interpreter.halt(.stack_overflow);
                return;
        ...
        pub fn opReturndatacopy(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(3)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opOrigin(ctx: *InstructionContext(DB)) void {
        ...
        pub fn opGasprice(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opCoinbase(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opTimestamp(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opNumber(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opDifficulty(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opGaslimit(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opChainid(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opBasefee(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opBlobhash(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opBlobbasefee(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
        ...
        pub fn opSlotnum(ctx: *InstructionContext(DB)) void {
            const h = ctx.host orelse {
                ctx.interpreter.halt(.invalid_opcode);
                return;
            };
