...
/// Bitwise opcode handlers, comptime-generic over DB — see host.zig's
...
pub fn Ops(comptime DB: type) type {
    return struct {
        ...
        pub fn opAnd(ctx: *InstructionContext(DB)) void {
        ...
        pub fn opOr(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// XOR opcode (0x18): a ^ b
        /// Stack: [a, b] -> [a ^ b]   Static gas: 3 (VERYLOW)
        pub fn opXor(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opNot(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(1)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opByte(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opShl(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// Stack: [shift, value] -> [value >> shift]   Static gas: 3 (VERYLOW)
        pub fn opShr(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opClz(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(1)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opSar(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
