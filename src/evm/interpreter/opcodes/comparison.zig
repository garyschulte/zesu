...
/// Comparison opcode handlers, comptime-generic over DB — see host.zig's
...
pub fn Ops(comptime DB: type) type {
    return struct {
        ...
        pub fn opLt(ctx: *InstructionContext(DB)) void {
        ...
        pub fn opGt(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// Stack: [a, b] -> [a < b ? 1 : 0]   Static gas: 3 (VERYLOW)
        pub fn opSlt(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// Stack: [a, b] -> [a > b ? 1 : 0]   Static gas: 3 (VERYLOW)
        pub fn opSgt(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        /// EQ opcode (0x14): a == b
        /// Stack: [a, b] -> [a == b ? 1 : 0]   Static gas: 3 (VERYLOW)
        pub fn opEq(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(2)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
        ...
        pub fn opIsZero(ctx: *InstructionContext(DB)) void {
            const stack = &ctx.interpreter.stack;
            if (!stack.hasItems(1)) {
                ctx.interpreter.halt(.stack_underflow);
                return;
